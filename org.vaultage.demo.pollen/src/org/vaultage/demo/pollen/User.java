...
public class User extends UserBase {
	...
	/**
	 * Map from the response token of a relayed request (to the next participant) to
	 * the request token of the previously received request (from the previous
	 * participant)
	 */
	private Map<String, String> responseToRequestMapper = new HashMap<>();

	// The following maps use the poll id to store the fake value and the final
	// answer
	...
	// operations
	@Override
	public void sendNumberPoll(String requesterPublicKey, String requestToken, NumberPoll poll) throws Exception {
		...
		if (publicKey.equals(poll.getOriginator())) {
			...
			RemoteUser remote = new RemoteUser(this, requesterPublicKey);
	...
	@Override
	public void sendMultivaluedPoll(String requesterPublicKey, String requestToken, MultivaluedPoll poll)
			throws Exception {
	}
	 ...
	 * 
	 * @param responseToken The token previously used to request an answer to the
	 *                      next participant of the poll
	 */
