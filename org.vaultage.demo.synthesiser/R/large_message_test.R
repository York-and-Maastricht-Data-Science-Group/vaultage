...
source_file <- paste("..", .Platform$file.sep, "largeMessageResults.csv", sep = "")
target_file <- paste("graphs", .Platform$file.sep, "largeMessageResults.pdf", sep = "")

# load data from file
raw <- read.table(source_file, sep = ",", header = TRUE)

# adjust values to kilos (K)
raw$MessageBytes <- raw$MessageBytes / 1000
...
legend_labels <- c(" Unencrypted", "Encrypted")
