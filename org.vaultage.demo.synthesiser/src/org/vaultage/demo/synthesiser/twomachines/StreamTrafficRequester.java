 ...
 */
public class StreamTrafficRequester {
...
//			LOCAL_IP = "144.32.196.129";
		...
		for (int n : numOfBytes) {
			StreamTrafficRequester trafficSimulation = new StreamTrafficRequester(numWorkers, n, true,
					true);
		...
		for (int n : numOfBytes) {
			StreamTrafficRequester trafficSimulation = new StreamTrafficRequester(numWorkers, n, false,
					true);
			for (int rep = 0; rep < numReps; rep++) {
				trafficSimulation.run();
				System.out.println(trafficSimulation.getLatestRunDetails());
	...
	public StreamTrafficRequester(int numWorkers, int numBytes, boolean brokered, boolean encrypted) {
